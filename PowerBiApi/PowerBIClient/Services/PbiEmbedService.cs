...
namespace Agility.OPSX.Api.PowerBIClient.Services
{
    public class PbiEmbedService
    {
        ...
        public Microsoft.PowerBI.Api.PowerBIClient GetPowerBIClient()
        {
            var tokenCredentials = new TokenCredentials(aadService.GetAccessToken(), "Bearer");
            return new Microsoft.PowerBI.Api.PowerBIClient(new Uri(urlPowerBiServiceApiRoot), tokenCredentials);
        }
        ...
        {
            Microsoft.PowerBI.Api.PowerBIClient pbiClient = this.GetPowerBIClient();

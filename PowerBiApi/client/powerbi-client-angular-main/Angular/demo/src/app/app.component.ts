...
export class AppComponent implements OnInit{
  ...
  getToken() {
    this.httpService.getEmbedConfig('http://localhost:5000/api/PowerBIEmbedInfo/GetEmbedInfo').subscribe((response) => {
      if (response) {
         //alert(JSON.stringify(response));
        alert(response.EmbedReport[0].EmbedUrl);
        const newUrl2 =response.EmbedReport[0].EmbedUrl.replace(/reportId=[^&]+/, `reportId=a32c45af-9500-4cbc-b582-e5871f8a2bfd`);
        alert(newUrl2);
        this.embedReport(response.EmbedToken.Token, response.EmbedReport[0].EmbedUrl);  
       // this.embedReport(response['embedToken'], response.embedReports[0].embedUrl); 
